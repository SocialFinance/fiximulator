...
my $exitCode = dockerize($TAR_PATH, $CONTAINER, $VERSION, $pushBuild);
...
sub dockerize {
    my ($path2tar, $container, $version, $pushIt) = @_;
    ...
    eval {
        ...
        my $dockerFile = dockerFile($hasEntry, $hasCmd, $hasVersion, $tarname);
...
sub dockerFile {
    my ($hasEntry, $hasCmd, $hasVersion, $tarName) = @_;
